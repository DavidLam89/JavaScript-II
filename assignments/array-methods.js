...
ticketPriceTotal = runners.reduce((total, x) => total + x.donation, 0);
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 Businesses with donations amount to $200 and above get to have their signs displayed.
let largeDonations = [];
largeDonations = runners.filter(x => x.donation >= 200);
console.log(largeDonations);

// Problem 2 Thank you letters for donations amount to $200 and above. Emails, company names, and representatives' last names are needed.
let thankYouLetters = [];
largeDonations.forEach(function(element) {
    thankYouLetters.push(`${element.last_name} from ${element.company_name}: ${element.email}`);
});
console.log(thankYouLetters);

// Problem 3 To cut cost, representatives from companies with donations below $50 will be given candies instead of t-shirts.
let tinyDonations = [];
runners.forEach(function(x){
    if (x.donation < 50) {
        x.shirt_size = "no shirt, candy only";
        tinyDonations.push(x); 
    }
});
